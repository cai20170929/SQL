...
public class HW6 {
  ...
  public static void main(String[] args) {
    ...
    r1.javaRDD().repartition(1).saveAsTextFile("output1");

    /* Problem 2 */
    JavaRDD<Row> r2 = Q2(spark, dataFile);
    r2.repartition(1).saveAsTextFile("output2");

    /* Problem 3 */
    JavaPairRDD<Tuple2<String, Integer>, Integer> r3 = Q3(spark, dataFile);
    r3.repartition(1).saveAsTextFile("output3");
    ...
    Tuple2<String, Integer> r4 = Q4(spark, dataFile);
      ...
      .javaRDD().saveAsTextFile("output4");


    /* Problem 5 */
     JavaPairRDD<String, Double> r5 = Q5(spark, dataFile);
     r5.repartition(1).saveAsTextFile("output5");
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    JavaPairRDD<Tuple2<String, Integer>, Integer> result =
      d.filter(t -> !t.get(CANCELLED).toString().equals("1"))
      .mapToPair(t -> new Tuple2<Tuple2<String, Integer>, Integer>(
                      new Tuple2<String, Integer>(t.get(ORIGIN_CITY_NAME).toString(),
                                                  Integer.valueOf(t.get(MONTH).toString())),
                      1))
      .reduceByKey((a,b)->a+b);
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> d = df.javaRDD();

    JavaPairRDD<String,String> dis_pair =
      d.mapToPair(t -> new Tuple2<String, String>(t.get(ORIGIN_CITY_NAME).toString(),
                                                  t.get(DEST_CITY_NAME).toString()))
      .distinct();

    JavaPairRDD<String,Integer> pair_count =
      dis_pair.mapToPair(t -> new Tuple2<String, Integer>(t._1, 1))
      .reduceByKey((a,b)->a+b);

    Tuple2<String, Integer> max_city =
      pair_count.max(new Compare());

    long total_f =
      d.filter(t -> t.get(ORIGIN_CITY_NAME).equals(max_city._1))
      .count();
    
//    df.createOrReplaceTempView("flights");
//    Dataset<Row> r = spark.sql("select distinct f.origincityname as origin, count(distinct destcityname) as count, count(*) as total from flights as f group by f.origincityname order by count desc limit 1");
//    r.show();

    return new Tuple2<String, Integer>(max_city._1, (int)total_f);
  }
    
  private static class Compare implements Serializable, Comparator<Tuple2<String, Integer>>{
    @Override
    public int compare(Tuple2<String, Integer> a1, Tuple2<String, Integer> a2) {
      return Integer.compare(a1._2, a2._2);
    }
  }
    
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {

    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD().filter(t -> t.get(DEP_DELAY) != null);
 
    JavaPairRDD<String,Double> pair_count =
      d.mapToPair(t -> new Tuple2<String, Double>(t.get(ORIGIN_CITY_NAME).toString(), 
                                                  Double.valueOf(t.get(DEP_DELAY).toString())))
      .reduceByKey((a,b)->a+b);
      
    JavaPairRDD<String,Double> total_count =
      d.mapToPair(t -> new Tuple2<String, Double>(t.get(ORIGIN_CITY_NAME).toString(), 1.0))
      .reduceByKey((a,b)->a+b);
      
    JavaPairRDD<String,Double> result =
      pair_count.join(total_count)
